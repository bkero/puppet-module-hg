...
cd /repo_local/mozilla/users
for i in $(find . -maxdepth 1 -type d -printf '%f\n')
do
    unset USERDIR
    export USERDIR=/repo_local/webroot_wsgi/users/$i
    ...
    then
        ...
        echo -e "/ = /repo_local/mozilla/users/$i/*"          >> hgweb.config
    fi

    if [ \! -f $USERDIR/hgweb.wsgi ]
    then
        echo    "#!/usr/bin/env python"                                      > hgweb.wsgi
        echo -e "config = '/repo_local/webroot_wsgi/users/$i/hgweb.config'" >> hgweb.wsgi
